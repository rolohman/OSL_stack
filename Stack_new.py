...
class run(object):
    ...
    def configure(self,inps, runName):
        ...
        self.runf= open(self.run_outname,'a+')
        
        #construct runstuff file
        notFound=True
        if os.path.isfile(self.work_dir+"/runstuff"):
            with open(self.work_dir+"/runstuff","r") as file:
                for line in file:
                    if re.search(runName, line):
                        notFound=False
    ...
    def unpackStackReferenceSLC(self, safe_dict):
        if self.polarization=='vv' or (self.polarization=='vh' and re.search('SDV',safe_dict[self.reference_date].safe_file)): 
                #check to make sure its dual-pol
            swath_path = self.work_dir
            os.makedirs(self.config_path, exist_ok=True)
            configName = os.path.join(self.config_path,'config_reference_'+self.polarization)
            configObj = config(configName)
            configObj.configure(self)
            ...
            configObj.swaths = self.swath_num
            configObj.outDir = os.path.join(self.work_dir, 'reference'+'/'+self.polarization)
    ...
    def unpackSecondarysSLC(self,  stackReferenceDate, secondaryList, safe_dict):
        ...
        for secondary in secondaryList:
            if self.polarization=='vv' or (self.polarization=='vh' and re.search('SDV',safe_dict[secondary].safe_file)): 
                #check to make sure its dual-pol
                configName = os.path.join(self.config_path,'config_secondary_'+secondary+'_'+self.polarization)
                outdir = os.path.join(self.work_dir,'secondarys/'+self.polarization+'/'+secondary)
                configObj = config(configName)
                configObj.configure(self)
    ...
    def averageBaseline(self, stackReferenceDate, secondaryList):
        ...
        for secondary in secondaryList:
            ...
            configObj = config(configName)
            configObj.configure(self)
            configObj.reference = os.path.join(self.work_dir,'reference/vv/')
            configObj.secondary = os.path.join(self.work_dir,'secondarys/vv/'+secondary)
    ...
    def gridBaseline(self, stackReferenceDate, secondaryList):
        ...
        configObj.configure(self)
        configObj.reference = os.path.join(self.work_dir,'reference/vv/')
        configObj.secondary = os.path.join(self.work_dir,'reference/vv/')
    ...
    def extractOverlaps(self):
        ...
        self.runf.write(self.text_cmd + 'subsetReference.py -m ' + os.path.join(self.work_dir, 'reference/vv') + ' -g ' + os.path.join(self.work_dir, 'geom_reference') + '\n')
    ...
    def geo2rdr_offset(self, secondaryList, fullBurst='False'):
        ...
        for secondary in secondaryList:
            ...
            if fullBurst == 'True':
                configName = os.path.join(self.config_path, 'config_fullBurst_geo2rdr_' + secondary+'_'+self.polarization)
            else:
                configName = os.path.join(self.config_path, 'config_overlap_geo2rdr_'+secondary+'_'+self.polarization)
            ###########
            configObj = config(configName)
            configObj.configure(self)
            configObj.secondaryDir = os.path.join(self.work_dir, 'secondarys/'+self.polarization+'/'+secondary)
            configObj.referenceDir = os.path.join(self.work_dir, 'reference/'+self.polarization)
            configObj.geom_reference = os.path.join(self.work_dir, 'geom_reference')
            configObj.coregSecondaryDir = os.path.join(self.work_dir, 'coreg_secondarys/'+self.polarization+'/'+secondary)
            if fullBurst == 'True':
    ...
    def resample_with_carrier(self, secondaryList, fullBurst='False'):
        ...
        for secondary in secondaryList:
            reference = self.reference_date
            if fullBurst == 'True':
                configName = os.path.join(self.config_path, 'config_fullBurst_resample_' + secondary+'_'+self.polarization)
            else:
                configName = os.path.join(self.config_path, 'config_overlap_resample_' + secondary+'_'+self.polarization)
            ###########
            configObj = config(configName)
            configObj.configure(self)
            configObj.secondaryDir = os.path.join(self.work_dir, 'secondarys/'+self.polarization+'/' + secondary)
    ...
    def pairs_misregistration(self, dateList, safe_dict):
        ...
        for date in dateList:
            safe_dict[date].slc = os.path.join(self.work_dir , 'coreg_secondarys/vv/'+date)
            safe_dict[date].slc_overlap = os.path.join(self.work_dir , 'coreg_secondarys/vv/'+date)
        safe_dict[self.reference_date].slc = os.path.join(self.work_dir , 'reference/vv')
        safe_dict[self.reference_date].slc_overlap = os.path.join(self.work_dir , 'reference/vv')
    ...
    def generate_burstIgram(self, dateList, safe_dict, pairs):
        ...
        for date in dateList:
            safe_dict[date].slc = os.path.join(self.work_dir, 'coreg_secondarys/'+self.polarization+'/'+date)
        safe_dict[self.reference_date].slc = os.path.join(self.work_dir , 'reference/'+self.polarization)
    ...
    def mergeSecondarySLC(self, secondaryList, virtual='True'):
        ...
        line_cnt = 0
        for secondary in secondaryList:
            configName = os.path.join(self.config_path,'config_merge_' + secondary+'_'+self.polarization)
            configObj = config(configName)
            configObj.configure(self)
            configObj.stack = os.path.join(self.work_dir, 'stack')
            configObj.reference = os.path.join(self.work_dir, 'coreg_secondarys/'+self.polarization+'/' + secondary)
            configObj.dirName = configObj.reference
            configObj.namePattern = 'burst*slc'
            configObj.mergedFile = os.path.join(self.work_dir, 'merged/SLC_'+self.polarization+'/' + secondary + '/' + secondary + '.slc')
            configObj.mergeBurstsMethod = 'top'
            configObj.aligned = 'True'
            configObj.validOnly = 'True'
    ...
    def mergeReference(self, stackReference, virtual='True'):
        ...
        configName = os.path.join(self.config_path,'config_merge_' + stackReference + '_' + self.polarization)
        configObj = config(configName)
        configObj.configure(self)
        configObj.stack = os.path.join(self.work_dir, 'stack')
        ...
        configObj.dirName = configObj.reference
        configObj.namePattern = 'burst*slc'
        configObj.mergedFile = os.path.join(self.work_dir, 'merged/SLC_'+ self.polarization +'/' + stackReference + '/' + stackReference + '.slc')
        configObj.mergeBurstsMethod = 'top'
    ...
    def finalize(self):
        self.runf.close()
        os.chmod(self.run_outname,stat.S_IRWXU)
