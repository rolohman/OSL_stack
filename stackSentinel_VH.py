...
from Stack_new import config, run, sentinelSLC
...
def slcStack(inps, acquisitionDates, stackReferenceDate, secondaryDates, newAcquisitions, safe_dict, updateStack, mergeSLC=False):
    #############################
    i=0
    ...
    runObj.unpackSecondarysSLC(stackReferenceDate, newAcquisitions, safe_dict)
    runObj.finalize()
    ...
    runObj.averageBaseline(stackReferenceDate, newAcquisitions)
    runObj.finalize()
    ...
    if inps.coregistration in ['NESD', 'nesd']:
        ...
        runObj.geo2rdr_offset(newAcquisitions)
        runObj.finalize()

        i+=1
        runObj = run()
        runObj.configure(inps, 'run_{:02d}_overlap_resample'.format(i))
        runObj.resample_with_carrier(newAcquisitions)
        runObj.finalize()
    ...
    runObj.geo2rdr_offset(newAcquisitions, fullBurst='True')
    runObj.finalize()
    ...
    runObj.resample_with_carrier(newAcquisitions, fullBurst='True')
    runObj.finalize()
    ...
    if mergeSLC:
        ...
        runObj.gridBaseline(newAcquisitions, secondaryDates)
        runObj.finalize()


    return i

def interferogramStack(inps, acquisitionDates, stackReferenceDate, secondaryDates, newAcquisitions, safe_dict, pairs, updateStack):

    i = slcStack(inps, acquisitionDates, stackReferenceDate, secondaryDates, newAcquisitions, safe_dict, updateStack)
    ...
    runObj.write('rm -rf '+inps.work_dir+'/ESD\n')
    runObj.write('rm -rf '+inps.work_dir+'/coarse_interferograms\n')
    runObj.write('rm -rf '+inps.work_dir+'/interferograms\n')
    ...
    os.chmod(inps.work_dir+'/runstuff2',stat.S_IRWXU)
...
def checkCurrentStatus(inps):
    ...
    coregSLCDir = os.path.join(inps.work_dir, 'coreg_secondarys/vv') #look in vv since that should be complete
    stackUpdate = False
    if os.path.exists(coregSLCDir):
        ...
        newAcquisitions = acquisitionDates
        print('No existing stack was identified. A new stack will be generated.')

    return acquisitionDates, stackReferenceDate, secondaryDates, newAcquisitions, safe_dict, stackUpdate
...
def main(iargs=None):
    ...
    acquisitionDates, stackReferenceDate, secondaryDates, newAcquisitions, safe_dict, updateStack = checkCurrentStatus(inps)
    ...
    if inps.workflow == 'interferogram':
        ...
        interferogramStack(inps, acquisitionDates, stackReferenceDate, secondaryDates, newAcquisitions, safe_dict, pairs, updateStack)
